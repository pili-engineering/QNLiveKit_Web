...
export class MockApi {
  ...
  static getAdminCensorConfig(params?: GetAdminCensorConfigParams) {
    return mockRequest<GetAdminCensorConfigResult['data']>(Mock.mock({
      enable: '@boolean',
      pulp: '@boolean',
      terror: '@boolean',
      politician: '@boolean',
  ...
  static getAdminCensorLive(params: GetAdminCensorLiveParams) {
    ...
    const count = current === last && total % pageSize !== 0 ? total % pageSize : pageSize;
    const page_total = total % pageSize === 0 ? total / pageSize : Math.floor(total / pageSize) + 1;
    ...
    const mockList = {
      list: list.list.map((item: Required<Required<GetAdminCensorLiveResult>['data']>['list'][number]) => {
        return {
          ...item,
  ...
  static getAdminCensorRecord(params: GetAdminCensorRecordParams) {
    ...
    const mockList = {
      list: list.list.map((item: Required<Required<GetAdminCensorRecordResult>['data']>['list'][number]) => {
        return {
          ...item,
          created_at: item.created_at ? new Date(item.created_at).getTime() : null,
          review_time: item.review_time ? new Date(item.review_time).getTime() : null,
        };
      })
    };
