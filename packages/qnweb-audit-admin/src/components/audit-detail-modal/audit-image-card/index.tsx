...
export const AuditImageCard: React.FC<AuditImageCardProps> = (props) => {
  const {
    createdAt, url, checkboxVisible = true, checked = false, cover, tags = [],
    onChange,
  } = props;

  const tooltipTitle = tags.map(item => {
  ...
  >
    <div className={`${prefixCls}-body`}>
      <Tooltip title={createdAt ? moment(createdAt).format('YYYY-MM-DD HH:mm:ss') : null}>
        <Image
      ...
      {
        checkboxVisible && <Checkbox checked={checked} onChange={onChange} className={`${prefixCls}-checkbox`}>
