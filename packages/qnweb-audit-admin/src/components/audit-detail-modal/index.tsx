...
export type AuditDetailModalProps = ModalProps & {
	 ...
	 */
	onPassClick?: (
		values: listRequestServiceResultListItem[],
		callback?: Callbacks
	) => void;
	/**
	 * 点击违规按钮
...
type listRequestServiceResultListItem =
	listRequestServiceResult['list'][number];
...
export const AuditDetailModal: React.FC<AuditDetailModalProps> = (props) => {
	const {
		...
		listRequestService,
		livePreviewProps,
		onEndLiveClick: handleEndLiveClick,
		onPassClick: handlePassClick,
		onViolationClick: handleViolationClick,
		...restProps
	} = props;

	const { loading, data, pagination, runAsync, refresh } = usePagination<
		listRequestServiceResult,
		Parameters<ListRequestService>
	>(listRequestService, {
		manual: true
	});
	const [checkedCards, setCheckedCards] = useState<
		listRequestServiceResultListItem[]
	>([]);
	...
	const checkedAll = !!(
		data?.list.length && checkedCards.length >= data?.list.length
	);
			...
			<Spin spinning={loading}>
				...
				<Space direction="vertical" size={32} style={{ width: '100%' }}>
					<div className={`${prefixCls}-header`}>
						<Radio.Group value={tab} onChange={onTabChange}>
						...
						{isClosed ? null : (
							<Button type="primary" danger={true} onClick={onEndLiveClick}>
								结束直播
							</Button>
						)}
					</div>
					...
					{listTab.includes(tab) ? (
						data?.list.length ? (
							<Space wrap={true} size={[36, 24]}>
								{data?.list.map((item) => {
									return (
										<AuditImageCard
											key={item.id}
											createdAt={
												tab === 'screenshotRecord' ? item.createdAt : null
											}
					...
					{tab === 'livePreview' ? (
						<div className={`${prefixCls}-live-preview`}>
							<div style={{ display: isPlayerVisible ? 'block' : 'none' }}>
								<div
									id="player"
									className={`${prefixCls}-live-preview-player`}
								/>
								<div style={{ textAlign: 'center', paddingTop: 24 }}>
									直播总时长：{livePreviewProps?.duration}
								</div>
							</div>
							<div
								style={{
					...
					>
						...
						{tab === 'todo' ? (
							<Space>
								...
								<Space size={20}>
									<Button onClick={onPassClick} disabled={isForceClosed}>
										通过
									</Button>
									<Button onClick={onViolationClick} disabled={isForceClosed}>
										违规
									</Button>
								</Space>
							</Space>
						...
						{listTab.includes(tab) ? (
							<Pagination simple={true} {...pagination} />
						) : null}
					</div>
				</Space>
			</Spin>
