...
export const BasicTable: React.FC<BasicTableProps> = (props) => {
  ...
  const columns: BasicTableProps['columns'] = [
    {
      title: '直播间', dataIndex: 'title', key: 'title',
      render: (text) => <Tooltip className={`${prefixCls}-title`} title={text}>
        {text.length > 10 ? `${text.slice(0, 10)}...` : text}
      </Tooltip>
    },
    { title: '主播', dataIndex: 'host', key: 'host' },
    {
      title: '当前状态', dataIndex: 'status', key: 'status', render: (_, row) => {
        const { status } = row;
        const statusCls = classNames(`${prefixCls}-status`, {
          [`${prefixCls}-status-${status}`]: status,
        });
        return <span className={statusCls}>{mapStatus[status || 'unknown']}</span>;
      }
    },
    { title: '待审核记录', dataIndex: 'todoRecords', key: 'todoRecords' },
    {
      title: '最近一次预警时间', dataIndex: 'lastWarningTime', key: 'lastWarningTime',
      render: (value) => value ? moment(value).format('YYYY-MM-DD HH:mm:ss') : '-',
    },
    {
      title: 'AI预警次数', dataIndex: 'aiWarningCount', key: 'aiWarningCount',
      render: (value) => value || 0,
    },
    {
      title: '违规次数', dataIndex: 'illegalCount', key: 'illegalCount',
      render: (value) => value || 0,
    },
    {
      title: '操作',
      ...
      render: (_, row) => {
        return row.status !== 'pending' ?
          <Button type="link" onClick={() => onDetailClick?.(row)}>查看详情</Button> : '-';
      }
    },
  ];
  ...
  return <Table
    ...
    columns={columns.filter(item => filterColumns?.includes(item.key as string))}
    rowKey="id"
    pagination={{
