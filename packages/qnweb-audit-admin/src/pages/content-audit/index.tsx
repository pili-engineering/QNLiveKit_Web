...
import { Layout, Space, Switch, Radio, RadioChangeEvent, Button, message, Spin, Modal, Checkbox } from 'antd';
...
const warningAlertMessage = (datetime?: number) => {
  return <span
    style={{ color: '#EF4149' }}
  >
    直播间涉嫌违规，管理员于{moment(datetime).format('YYYY/MM/DD HH:mm:ss')}强制关闭
  </span>;
};
...
export const ContentAudit = () => {
  ...
  const {
    ...
    return AdminApi.getAdminCensorLive(paramsMap[tableTab || 'todo']).then(result => {
   ...
   * 获取审核设置
   */
  const { loading: settingConfigLoading, data: settingConfig, refresh: refreshSettingConfig } = useRequest(() => {
  ...
  const {
    ...
    runAsync: runUpdateSettingAsync
  } = useRequest<PostAdminCensorConfigResult['data'], PostAdminCensorConfigParams[]>((params) => {
    return AdminApi.postAdminCensorConfig(params).then(result => result.data);
  }, {
    manual: true,
  });
  /**
   * 结束直播
   */
  const { runAsync: runStopLiveAsync } = useRequest<PostAdminCensorStopliveLiveIdResult, [PostAdminCensorStopliveLiveIdParams]>((params) => {
   ...
   * 审核
   */
  const { runAsync: runAuditAsync } = useRequest<PostAdminCensorAuditResult, [PostAdminCensorAuditParams]>((params) => {
   ...
   * 直播列表数据清洗
   */
  const basicTableDataSource: BasicTableDataType[] = (liveTableData?.list || []).map(item => {
    return {
      ...
      liveTotalTime: moment.utc(
        (
          (item.stop_at || Date.now()) - (item.start_at || Date.now())
        )
      ).format('HH小时mm分钟'),
    };
  });
   ...
   * @param params
   */
  const listRequestService: AuditDetailModalProps['listRequestService'] = (params) => {
   ...
   * 结束直播
   */
  const onEndLiveClick: AuditDetailModalProps['onEndLiveClick'] = (callback) => {
    Modal.confirm({
      icon: null,
      title: '提示',
      content: '是否强制结束直播？',
   ...
   * 通过
   */
  const onPassClick: AuditDetailModalProps['onPassClick'] = (values, callback) => {
   ...
   * 违规
   */
  const onViolationClick: AuditDetailModalProps['onViolationClick'] = (values, callback) => {
    ...
    Modal.confirm({
      icon: null,
      title: '提示',
      content: <div className="violation-modal-content">
        <div>选中的图片将被标记为“违规”，是否继续？</div>
        <Checkbox>同时向直播间发出警告</Checkbox>
      </div>,
      onOk() {
        const checked = !!document.querySelector('.violation-modal-content .ant-checkbox-checked');
        return runAuditAsync({
          live_id: curRow?.live_id,
          image_list: values.map(item => +item.id),
          review_answer: 2,
   ...
   * 强制关闭的直播间alert提示
   */
  const auditDetailModalAlertProps = (): AuditDetailModalProps['alertProps'] => {
    if (curRow?.status !== 'forceClosed') return null;
    const forceCloseLive = liveTableData?.list.find(
      item => item.live_id === curRow?.live_id
    );
    const forceCloseDatetime = forceCloseLive ? forceCloseLive.stop_at : Date.now();
    return {
   ...
   * table columns map
   */
  const filterColumnsMap: Record<TableTabOptValue, BasicTableProps['filterColumns']> = {
    todo: ['title', 'host', 'status', 'todoRecords', 'lastWarningTime', 'action'],
    ...
    <Layout className={styles.container}>
      <AuditSettingModal
        initialValues={{
          ...
          tags: Array.from(new Set((getSettingTags(settingConfig) || []).concat('pulp'))),
        }}
      ...
      <AuditDetailModal
        ...
        livePreviewProps={{ url: curRow?.liveUrl, duration: curRow?.liveTotalTime }}
