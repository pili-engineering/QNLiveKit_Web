...
const renderTextMessage = (message: ChatBoxListItem) => {
  return <div className={`${prefixCls}-message`} key={message.id}>
    <Avatar
      src={message.avatar}
      size={24}
    />
    <div className={`${prefixCls}-message-content`}>
      <div className={`${prefixCls}-message-content-username`}>{message.username}</div>
      <div className={`${prefixCls}-message-content-text`}>{message.content}</div>
    </div>
  </div>;
};

const renderGiftMessage = (message: ChatBoxListItem) => {
  return <div className={`${prefixCls}-message-gift`} key={message.id}>
    <Avatar
      src={message.avatar}
      size={24}
    />
    <div className={`${prefixCls}-message-gift-content`}>
      <div className={`${prefixCls}-message-gift-content-username`}>{message.username}</div>
      <div>打赏</div>
      <div className={`${prefixCls}-message-gift-content-text`}>{message.content}</div>
    </div>
  </div>;
};
...
export const ChatBox: React.FC<ChatBoxProps> = (props) => {
  const {
    className, style,
    list, scrollHeight, autoScroll = true
  } = props;
  ...
  return (
    <div
      ...
      style={{ maxHeight: scrollHeight, overflowY: scrollHeight ? 'scroll' : 'auto', ...style }}
      ref={chatBoxEleRef}
    >
