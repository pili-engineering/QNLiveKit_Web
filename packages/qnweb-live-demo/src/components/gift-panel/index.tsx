...
export const GiftPanel: React.FC<GiftPanelProps> = (props) => {
    const {
      className, style,
      visible = true, title, selectedGift, list = [], fixedBottom,
      onClose, onItemClick, onGiftPayClick
    } = props;
      ...
      <div className={`${prefixCls}-list`}>
        {
          list.map((item) => {
            ...
            >
              ...
              {
                selected ? <div className={`${prefixCls}-list-item-selected-pay`} onClick={() => onGiftPayClick?.(item)}>
                  <div className={`${prefixCls}-list-item-selected-pay-title`}>{item.price}</div>
                  <div className={`${prefixCls}-list-item-selected-pay-button`}>支付</div>
                </div> : <div className={`${prefixCls}-list-item-text`}>
                  <div className={`${prefixCls}-list-item-title`}>{item.title}</div>
                  <div className={`${prefixCls}-list-item-price`}>{item.price}</div>
                </div>
              }
            </div>;
          })
