...
export const ProductCard: React.FC<ProductCardProps> = (props) => {
  const {
    className, style,
    thumbnail, order, title, tags, originalPrice, currentPrice, isLive, recordUrl,
    onBuyClick, onRecordClick
  } = props;

  const renderTip = () => {
    if (isLive) {
      return <div className={`${prefixCls}-live`}>
        <LiveAnimation/>
        <span className={`${prefixCls}-live-text`}>讲解中</span>
      </div>;
    }
    if (recordUrl) {
      return <div className={`${prefixCls}-record`} onClick={onRecordClick}>
        <PlayCircleOutlined/>
    ...
    <div className={`${prefixCls}-content`}>
      <div className={`${prefixCls}-header`}>
        ...
        <div className={`${prefixCls}-tags`}>
          {
            tags?.map((tag, index) => {
              return <Tag key={index} color="#fa8c15">{tag}</Tag>;
            })
          }
        </div>
      </div>
      ...
      <div className={`${prefixCls}-footer`}>
        <div className={`${prefixCls}-price`}>
          {
            currentPrice ? <span className={`${prefixCls}-price-current ${prefixCls}-price-large`}>
              {currentPrice}
            </span> : null
          }
          {
            originalPrice ? <span className={`${prefixCls}-price-original`}>
              {originalPrice}
            </span> : null
          }
        </div>

        <Button type="primary" danger onClick={onBuyClick}>去购买</Button>
      </div>
    </div>
  </div>;
};
