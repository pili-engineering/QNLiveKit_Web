...
import {
  ChatBox,
  ChatBoxListItem, GiftAnimation, GiftAnimationHandler,
  GiftPanel, GiftPanelProps, IconBarrage,
  IconChatSvg,
  IconLike, IconLikeProps,
  IconMoreSvg,
  IconShoppingCartSvg, LiveTimer,
  MorePanel,
  ProductList,
  ProductListProps,
  RoomHeader, WaitingNotice
} from '@/components';
import { curConfig } from '@/config';
import { useIMStore, useUserStore } from '@/store';
import { getUrlQuery, QNIMManager, roomStatusAdapter, XGPlayer, XGPlayerConstructor } from '@/utils';
...
export const LiveRoom: React.FC = () => {
  ...
  const createMsg = (data: {
    ...
    if (['liveroom-pubchat', 'liveroom-welcome', 'living_danmu'].includes(action)) {
    ...
    if (['gift_notify'].includes(action)) {
      const gift = giftData?.data?.find(item => `${item.gift_id}` === selectedGift);
      sendGiftRun({
        live_id: urlQueryRef.current.liveId,
  ...
  const handleRtmChannelListener = (result: string) => {
    ...
    if (isGiftMessage(json)) {
      const user = userList?.find(item => `${item.user_id}` === `${json.data?.user_id}`);
      const gift = giftData?.data?.find(item => `${item.gift_id}` === `${json.data?.gift_id}`);
  ...
  const handleRtmChannelListenerRef = useRef<typeof handleRtmChannelListener | null>(null);
   ...
   * 初始化im=>登录im=>加入聊天室
   */
  const { runAsync: initIMRunAsync } = useRequest<unknown, Array<{
    appid: string;
    channelId: string;
  }>>((params) => {
    ...
    const imClient = imClientRef.current;
    if (!imUsername) return Promise.reject(new Error(`imUsername is ${imUsername}`));
    if (!imPassword) return Promise.reject(new Error(`imPassword is ${imPassword}`));
    if (!imClient) return Promise.reject(new Error(`imClient is ${imClient}`));
  ...
  return <div className={styles.page}>
    ...
    {
      roomStatus === 'pause' || roomStatus === 'trailer' ? <WaitingNotice
    ...
    <ProductList
      ...
      fixedBottom={true}
      list={productList}
      onItemRecordClick={result => history.push(`/record?liveId=${urlQueryRef.current.liveId}&productId=${result.id}`)}
      onItemBuyClick={result => history.push(`/shop?liveId=${urlQueryRef.current.liveId}&productId=${result.id}`)}
    ...
    <div className={styles.footer}>
      ...
      {
        isInputFocus ? null : <div className={styles.icons}>
          <IconBarrage
