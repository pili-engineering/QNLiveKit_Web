...
import { IconShoppingCartSvg, ProductList, ProductListProps, ProductPopup, RoomHeader } from '@/components';
...
export const Record: React.FC<RecordProps> = (props) => {
   ...
   * @param result
   */
  const onProductRecordClick: ProductListProps['onItemRecordClick'] = (result) => {
    history.replace(`/record?liveId=${urlQueryRef.current.liveId}&productId=${result.id}`);
  ...
  const onProductPopupClick = () => {
    ...
    history.push(`/shop?liveId=${urlQueryRef.current.liveId}&productId=${currentProduct?.item_id}`);
  };
  ...
  return <div className={classNames(prefixCls, className)} style={style}>
    ...
    <div className={`${prefixCls}-footer`}>
      <Button
        ...
        icon={<LeftOutlined/>}
        block={true}
        onClick={() => history.replace(`/live-room?liveId=${urlQueryRef.current.liveId}`)}
      >
        返回当前直播
      </Button>
