...
export const RoomList = () => {
  ...
  const { loading, loadingMore, data, noMore, loadMoreAsync } = useInfiniteScroll<GetLiveRoomListResult['data']>(
    result => {
  ...
  return (
    <div className={styles.page}>
      {
        !loading && !data?.list.length ? <Empty className={styles.pageEmpty}/> : <InfiniteScroll
          dataLength={data?.list.length || 0}
          next={loadMoreAsync}
          hasMore={!noMore}
          loader={
            loading || loadingMore ? <div className={classNames({
          ...
          <div className={styles.list}>
            {
              data?.list.map(item => {
                return <RoomCard
                  ...
                  status={roomStatusAdapter({ liveStatus: item.live_status, anchorStatus: item.anchor_status })}
                  trailerTime={item.start_time * 1000}
                  onClick={() => history.push(`/live-room?liveId=${item.live_id}`)}
                />;
              })
            }
          </div>
