...
export const Shop: React.FC<ShopProps> = (props) => {
  ...
  const [currentProduct, setCurrentProduct] = useState<Required<GetClientItemLiveIdResult>['data'][number] | null>();

  /**
   * 根据id查商品
   */
  const getProductById = useCallback((id: string) => {
    return productListData?.data?.find(
  ...
  return <div className={classNames(prefixCls, className)} style={style}>
    <LeftCircleOutlined className={`${prefixCls}-icon`} onClick={() => history.goBack()}/>

    <Image style={{ height: '300px', objectFit: 'cover' }} preview={false} src={currentProduct?.thumbnail}/>
    <Image style={{ width: '80%', margin: '0 20px' }} preview={false} src={placeholderPNG}/>
    <div className={`${prefixCls}-footer`}>
      <Button className={`${prefixCls}-footer-button`} type="primary" danger={true}>立即购买</Button>
      <Button className={`${prefixCls}-footer-button`} type="primary">加入购物车</Button>
    </div>
  </div>;
};
